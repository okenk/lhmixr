...
vb_bind_gr <- function(theta, binding, data, distribution){
  ...
  if(distribution == "normal"){
    all.gradients <- c(
                       ## d/dlnlinfF
                       sum(-exp(lnlinfF-2*lnsigmaF)*weights*(1-exp(-exp(lnkF)*(age #+exp(lnnt0F)
                                                                               )))*(length-exp(lnlinfF)*(1-exp(-exp(lnkF)*(age#+exp(lnnt0F)
                                                                                                                             ))))),
                       ## d/dlnlinfM
                       sum(-exp(lnlinfM-2*lnsigmaM)*(1-weights)*(1-exp(-exp(lnkM)*(age#+exp(lnnt0M)
                                                                                   )))*(length-exp(lnlinfM)*(1-exp(-exp(lnkM)*(age#+exp(lnnt0M)
                                                                                                                               ))))),
                       ## d/dkF
                       sum(-exp(-2*lnsigmaF)*weights*(age#+exp(lnnt0F)
                                                      )*exp(-exp(lnkF)*(age#+exp(lnnt0F)
                                                                        )+lnkF+lnlinfF)*(length-exp(lnlinfF)*(1-exp(-exp(lnkF)*(age#+exp(lnnt0F)
                                                                                                                                ))))),
                       ## d/dkM
                       sum(-exp(-2*lnsigmaM)*(1-weights)*(age#+exp(lnnt0M)
                                                          )*exp(-exp(lnkM)*(age#+exp(lnnt0M)
                                                                            )+lnkM+lnlinfM)*(length-exp(lnlinfM)*(1-exp(-exp(lnkM)*(age#+exp(lnnt0M)
                                                                                                                                    ))))),
                       ## d/dlnnt0F
                       ...
                       ## d/dlnsigmaF
                       sum(weights*(1-exp(-2*lnsigmaF)*(length-exp(lnlinfF)*(1-exp(-exp(lnkF)*(age#+exp(lnnt0F)
                                                                                               ))))^2)),
                       ## d/dlnsigmaM
                       sum((1-weights)*(1-exp(-2*lnsigmaM)*(length-exp(lnlinfM)*(1-exp(-exp(lnkM)*(age#+exp(lnnt0M)
                                                                                                   ))))^2))
                       )
  }
  if(distribution == "lognormal"){
    all.gradients <- c(
                       ## d/dlnlinfF
                       sum(-exp(-2*lnsigmaF)*weights*(log(length)-log(exp(lnlinfF)*(1-exp(-exp(lnkF)*(age#+exp(lnnt0F)
                                                                                                      ))))+exp(2*lnsigmaF)/2)),
                       ## d/dlnlinfM
                       sum(-exp(-2*lnsigmaM)*(1-weights)*(log(length)-log(exp(lnlinfM)*(1-exp(-exp(lnkM)*(age#+exp(lnnt0M)
                                                                                                          ))))+exp(2*lnsigmaM)/2)),
                       ## d/dkF
                       sum(-exp(-2*lnsigmaF)*weights*(age#+exp(lnnt0F)
                                                      )*exp(lnkF-exp(lnkF)*(age#+exp(lnnt0F)
                                                                            ))*(log(length)-log(exp(lnlinfF)*(1-exp(-exp(lnkF)*(age#+exp(lnnt0F)
                                                                                                                                ))))+exp(2*lnsigmaF)/2)/(1-exp(-exp(lnkF)*(age#+exp(lnnt0F)
                                                                                                                                                                           )))),
                       ## d/dkM
                       sum(-exp(-2*lnsigmaM)*(1-weights)*(age#+exp(lnnt0M)
                                                          )*exp(lnkM-exp(lnkM)*(age#+exp(lnnt0M)
                                                                                ))*(log(length)-log(exp(lnlinfM)*(1-exp(-exp(lnkM)*(age#+exp(lnnt0M)
                                                                                                                                    ))))+exp(2*lnsigmaM)/2)/(1-exp(-exp(lnkM)*(age#+exp(lnnt0M)
                                                                                                                                                                               )))),
                       ## d/dlnnt0F
                       ...
                       ## d/dlnsigmaF
                       sum(-weights*(exp(-2*lnsigmaF)*(log(length)-log(exp(lnlinfF)*(1-exp(-exp(lnkF)*(age#+exp(lnnt0F)
                                                                                                       ))))+exp(2*lnsigmaF)/2)^2-log(length)+log(exp(lnlinfF)*(1-exp(-exp(lnkF)*(age#+exp(lnnt0F)
                                                                                                                                                                                 ))))-exp(2*lnsigmaF)/2-1)),
                       ## d/dlnsigmaM
                       sum(-(1-weights)*(exp(-2*lnsigmaM)*(log(length)-log(exp(lnlinfM)*(1-exp(-exp(lnkM)*(age#+exp(lnnt0M)
                                                                                                           ))))+exp(2*lnsigmaM)/2)^2-log(length)+log(exp(lnlinfM)*(1-exp(-exp(lnkM)*(age#+exp(lnnt0M)
                                                                                                                                                                                     ))))-exp(2*lnsigmaM)/2-1))
                       )
  }
